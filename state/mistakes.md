...
## 2026-09-15：零基 Warp 编号与二维题目条件

- 知识点：Block 内线性编号、Warp、Lane。
- 原回答：Thread70的Warp答3；线性编号50的Warp答2。二维8列题首次答26，提示中沿用上一题2*4+1=9。
- 原因：Warp编号混入第几组；提示过程暴露换题后沿用旧条件。不能仅凭首次26推断具体计算方式。
- 正确理解：各坐标从0开始；二维编号为y乘以x方向线程数再加x；除以32的整数商是Warp编号、余数是Lane。
- 复测：Thread105独立答Warp3；6列题独立写2*6+4=16；8列12行坐标(5,9)独立答77、2、13。提示后修正不覆盖首次评分；间隔后用解释题确认稳定性。
- 证据：[[sessions/2026-09-15-thread-index-relearning|本次记录]]。

## 2026-09-13：异步与无数据依赖

- 原回答：CPU解析B不必等GPU处理A，因为“都是异步执行的”。
- 正确部分：无需等待的判断正确。
- 缺口：尚未独立说明B不使用A结果这一条件；异步本身不消除数据依赖。
- 后续：提示后补充“不依赖结果”；保留待间隔独立复测，不改写历史已验证状态。
